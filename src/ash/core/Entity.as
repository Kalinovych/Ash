...
public class Entity {
	...
	protected var _componentObservers:LinkedHashSet;
	...
	public function Entity( name:String = "" ) {
		...
		_componentObservers = new LinkedHashSet();
		if ( name ) {
			_name = name;
		}
		else {
	...
	public function add( component:Object, componentClass:Class = null ):Entity {
		...
		// notify observers
		for (var node:ItemNode = _componentObservers.ecse::_firstNode; node; node = node.next) {
	...
	ecse function addComponentObserver( observer:IComponentObserver ):void {
		_componentObservers.add( observer );
	}
	
	ecse function removeComponentObserver( observer:IComponentObserver ):void {
